...
def parse_incels(output_dir, base_url, area_url):
    ...
    while current_forum_page.find('a', {'class': "pageNav-jump pageNav-jump--next"}) or final_page:
        ...
        for submission in tqdm.tqdm(current_forum_page.find_all('div', {'class': "structItem-cell structItem-cell--main"}), desc=f"Submission Page: {page}"):
            # 1. get all the metadata for the submission
            ...
            # Set up the data structures for the submission and user data
            submission_data = []
            ...
            if not submission.has_attr("data-preview-url"):
                # if not thread skip to next
                continue
            ...
            if first_page_submission_url in downloaded_submissions:
                ...
                print(f"{first_page_submission_url} already downloaded, skipping", file=sys.stderr)
                continue
            ...
            try:
                submission_title = submission_page.find_all("h1", {"class": "p-title-value"})[0].text.strip().replace('\xa0', ' ')
            except:
                print(f"Can't find missing title for {submission_page}, skipping", file=sys.stderr)
                continue
